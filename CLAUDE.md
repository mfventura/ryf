...
- `module/rolls/ryf-roll.mjs` — `RyfRoll` static methods (`rollSkill`, `rollAttack`, `rollDamage`, `rollSpellCasting`, `rollOpposed`, `rollDualDamage`, …) that perform 1o3d10 rolls and render the chat cards in `templates/chat/`. Negative factors (wounded, untrained) degrade the kept die via `degradeMode`; a natural 1 on the kept die always fails.
